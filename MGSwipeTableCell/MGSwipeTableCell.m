...
- (void) showSwipeOverlayIfNeeded
{
    ...
    CGSize  cropSize        = CGSizeMake(self.bounds.size.width, self.bounds.size.height);
    ...
    if (!_allowsMultipleSwipe) {
        ...
        MPTableView * table = [self parentTable];
...
-(void) setAccesoryViewsHidden: (BOOL) hidden
{
    for (UIView * view in self.subviews) {
        if (([view isKindOfClass:[UIButton class]] || [NSStringFromClass(view.class) rangeOfString:@"Disclosure"].location != NSNotFound)) {
            view.hidden = hidden;
        }
    }
...
-(MPTableView *) parentTable
{
    UIView * view = self.superview;
    while(view != nil) {
        if([view isKindOfClass:[MPTableView class]]) {
            return (MPTableView*) view;
        }
        view = view.superview;
    }
    return nil;
